...
class Level4Page(BaseLevelPage):
    def __init__(self, back_method):
        level_info = ("Dave can't afford a present for his son's birthday next week because he lost his job. He would "
                      ...
                      "values of the pins using pick_pin and return them in a list like the one above. However, "
                      "since it is so dark outside, lines in your code will go dark once you press enter on them!"
                      )
    ...
    @staticmethod
    def test_code(code, func_name, values_list, stdout_queue):
        buffer = io.StringIO()
